...
class PropDetail {
  ...
  //new fields for uploaded property 
  firstname?: string
  lastname?: string
  city?: string
  state?: string
...
export class UploadPropertyDetailComponent implements OnInit {
  ...
  constructor( private modal: NgbModal, private customerService: CustomerService, private router: ActivatedRoute, private route: Router, private fb: FormBuilder, private authService: AuthService, public sanitizer: DomSanitizer, private toastr: ToastrService) {
    ...
    this.authService.getuploadedDemograph(this.router.params['value'].id).subscribe((data) => {
      $('#cover-spin').hide(0);
    ...
    this.authService.getUploadedPropertyDetail(this.router.params['value'].id).subscribe((data) => {
      this.property = data.data
      this.demographData=data.data
