...
@Component({
  selector: 'app-highly-interested',
  templateUrl: './highly-interested.component.html',
  styleUrls: ['./highly-interested.component.scss'],
  encapsulation:ViewEncapsulation.None
})
export class HighlyInterestedComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  total:number=0;
  prospectList:any=[];
  btnSearchDisable:boolean = false;
  listenFunc: Function;
  ...
  constructor(private authService:AuthService, private actRouter:ActivatedRoute, private router:Router,elementRef: ElementRef, private renderer: Renderer, private modal: NgbModal, private toastr: ToastrService, private excelService:ExcelService,private store: Store<AppState>) {
  ...
  ngOnInit() {
    ...
    //this.authService.getHighlyInterestedAll().subscribe((data)=>{ 
      //this.exlTableData=data.data.data;
    //   var json = data.data.data;
    //  // var obj = JSON.parse(json)[0];
    //  var obj = json[0];
    //   obj['Owner1 First Name'] = obj.Owner1FirstName;
    //   delete obj.Owner1FirstName;
    //   obj['Owner1 Last Name'] = obj.OwnerLastname1;
    //   delete obj.OwnerLastname1;
    //   obj['Owner1 Type'] = obj.Owner1Type;
    //   delete obj.Owner1Type;
    //   obj['Owner1 Properties Owned'] = obj.Owner1PropertiesOwned;
    //   delete obj.Owner1PropertiesOwned;
    //   obj['Owner2 First Name'] = obj.Owner2FirstName;
    //   delete obj.Owner2FirstName;
    //   obj['Owner2 Last Name'] = obj.OwnerLastname2;
    //   delete obj.OwnerLastname2;
    //   json = JSON.stringify([obj]);
    //   this.exlTableData=JSON.parse(json);      
    //});
    this.authService.getGridList(2).subscribe((data)=>{ 
    ...
    this.dtOptions = {      
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.authService.getHighlyInterested({...dataTablesParameters, ...this.filterObj})
        .subscribe((data)=>{ 
  ...
  bulb(index:number): void{
    ...
    //   }
    //   if($this.find('.btnsLst li:nth-child(2) button').hasClass('bulbs2')){
  ...
  fire(index:number): void{
    ...
    //     $this = $('#prospectTbl tr:nth-child('+(index)+')');
    //   }
    //   if($this.find('.btnsLst li:nth-child(3) button').hasClass('fires2')){
  ...
  exportAsXLSX():void {
    this.authService.getAllHighlyInterestedPropertyExcel().subscribe((data)=>{ 
       this.exlTableData = data.data;
      this.excelService.exportAsExcelFile(this.exlTableData, 'Hot Prospects');
    });    
  }
