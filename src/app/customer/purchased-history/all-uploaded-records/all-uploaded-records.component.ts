...
@Component({
  selector: 'app-all-uploaded-records',
  templateUrl: './all-uploaded-records.component.html',
  styleUrls: ['./all-uploaded-records.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class AllUploadedRecordsComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ...
      searching: false,
      lengthMenu: [
        [10, 20, 25, 50, 100, 150, 200, 250, 300],
        [10, 20, 25, 50, 100, 150, 200, 250, 300]
      ],
      order: [[ 1, "desc" ]],
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.dataTables  = dataTablesParameters;
          this.authService.getUploadedRecords1({dataTablesParameters, ...this.filterObj}).subscribe((data)=>{
            $('#cover-spin').hide(0);
      ...
      columns: [
        ...
        // {
        //   data: 'status',
        //   title: 'Status', name: 'ser_property.status', orderable: false, visible: false,
        //   width: '30%',
        ...
        { title: 'Email & Phone', defaultContent: 'NA', orderable: false, 
          render: function (data, type, row) {
            ...
            if(row.email == ""){
              ...
              email = `<a  purchase-id="${row.id}" purchase-type="1" class="pending_btn not_found_records"><button class="btn btn-secondary secondary_tooltip"><i class="fa fa-envelope fa-lg"></i></button></a>`;
            }
              else{
          ...
          if(row.phone == 0 || row.phone ==''){
            phone = `<a purchase-id="${row.id}" purchase-type="2" class="pending_btn not_found_records"><button class="btn btn-secondary secondary_tooltip"><i class="fa fa-mobile fa-lg"></i></button> </a>`;
          }
          else{
        ...
        { title: 'Action', defaultContent: 'NA' , orderable: false, 
          render: function (data, type, row) {
            // return `<a href="/customer/property/${row.id}" target="_blank"><button class="btn btn-default">Go</button></a>`;
        ...
        {
          title: 'Address', width: '49',data: 'address', defaultContent: 'NA',
          render: function (data, type, row) {
        ...
        //     return row.LMSSalePrice ? row.LMSSalePrice : 'No Data' 
        // },  
        // }       
  ...
  exportAsXLSX(): void {
    ...
    this.authService.getUploadedRecords1({ dataTablesParameters }).subscribe((data)=>{
  ...
  //   dataTablesParameters = this.dataTables  
  //   dataTablesParameters.length = -1;  
  //   this.authService.getUploadedRecords1( {dataTablesParameters} )
  //     .subscribe((data) => {
  //       this.exlTableData =JSON.parse(data.data).data;
  //       if(this.rows_selected.length>0){
  //         const expData=[]
  //         this.exlTableData.map((item)=>{
  //             if(this.rows_selected.includes(item.result_id)){
  //               expData.push(item)
  //             }
  //         })
