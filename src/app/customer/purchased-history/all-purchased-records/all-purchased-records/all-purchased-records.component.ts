...
@Component({
  selector: 'app-all-purchased-records',
  templateUrl: './all-purchased-records.component.html',
  styleUrls: ['./all-purchased-records.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class AllPurchasedRecordsComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.authService.getGridList(9).subscribe((data)=>{ 
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.authService.getPurchaseRecords( {dataTablesParameters, ...this.filterObj} )
          .subscribe((data) => {
          ...
          this.data.forEach(element => {
            ...
            console.log(this.rows_selected, 'rows')
          });
  ...
  addSMSEmailToProspects(): void {
    ...
    if(!this.rows_selected.length || (!this.checkedText && !this.checkedEmail && !this.checkedPost) || this.proceedLoader){
      this.toastr.error('Please select atleast one row', 'Error!');
    }
    else if(type===''){
      if(this.rows_selected.length >0){
  ...
  sendMarketing1(arg , id){
      if(arg==1){
        this.authService.emailMarketingData(id,'email', 'datatree').subscribe(data=>{
          const modalRef = this.modal.open(EmailStatusComponent, { size: 'lg' });
          modalRef.componentInstance.data = data.data;
        ...
        this.authService.emailMarketingData(id,'phone','datatree').subscribe(data=>{
          console.log(data)
          const modalRef = this.modal.open(SmsStatusComponent, { size: 'lg' });
          modalRef.componentInstance.data = data.data;
          modalRef.componentInstance.rows = id;
