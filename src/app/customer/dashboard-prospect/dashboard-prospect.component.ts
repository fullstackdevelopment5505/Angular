...
@Component({
  selector: 'app-dashboard-prospect',
  templateUrl: './dashboard-prospect.component.html',
  styleUrls: ['./dashboard-prospect.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class DashboardProspectComponent implements OnInit {
  ...
  constructor(private authService: AuthService, private actRouter: ActivatedRoute,private toastr: ToastrService, private router: Router, elementRef: ElementRef, private renderer: Renderer, private modal: NgbModal) {
  ...
  ngOnInit() {
    ...
    this.dtOptionsHotProspect = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        //for hot
        this.authService.getHighlyInterested(Object.assign(dataTablesParameters, this.filterObj)).subscribe((data) => {
          let hotArr: any = [];
          this.prospectListHot = JSON.parse(data.data).data;
          for (var i = 0; i < 10; i++) {
            if (i < this.prospectListHot.length)
              hotArr.push(this.prospectListHot[i]);
          }
          this.prospectListHot = hotArr;
          callback({
            data: this.prospectListHot
          });
        }, error => {
          console.log(error)
        })
  ...
  trash(index: number): void {
    let info = this.prospectListHot[index];
  ...
  bulb(index:number): void{
    ...
    //   $this.find('.btn.btn-actions.fires').removeClass('fires').addClass('fires2');
      ...
      this.action.id = this.prospectListHot[index].property_id;
      this.authService.postAction(this.action)
      .subscribe((data) => { console.log(data) }, (error) => { console.log(error) });
  ...
  manageGridSubmit(form:any){    
    ...
    this.authService.setGridListHot(this.manageGrid).subscribe((data)=>{ 
